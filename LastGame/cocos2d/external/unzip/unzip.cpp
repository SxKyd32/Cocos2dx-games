...
#ifndef NOUNCRYPT
    uint32_t keys_2[3];                   /* keys defining the pseudo-random sequence */
...
ZEXTERN int ZEXPORT unzOpenCurrentFile3(unzFile file, int *method, int *level, int raw, const char *password)
{
    ...
    if ((password != NULL) && ((s->cur_file_info.flag & 1) != 0))
    {
...
#endif
        {
            ...
            init_keys(password, s->keys_2, s->pcrc_32_tab);

            if (ZREAD64(s->z_filefunc, s->filestream, source, 12) < 12)
                return UNZ_INTERNALERROR;

            for (i = 0; i < 12; i++)
                zdecode(s->keys_2, s->pcrc_32_tab, source[i]);
                ...
                if (s->pcrc_32_tab != NULL)
                {
                    ...
                    for (i = 0; i < total_bytes_read; i++)
                      s->pfile_in_zip_read->read_buffer[i] =
                          zdecode(s->keys_2, s->pcrc_32_tab, s->pfile_in_zip_read->read_buffer[i]);
                }
            }
#endif
